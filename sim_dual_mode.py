"""
python sim_dual_mode.py <path-to-config>
"""
...
from planner import TrajectoryOptimizerWholeBody, TrajectoryOptimizerBodySDF


class ObjCompDynamicDualModePlannerWithControl:
    def __init__(self, sdf_config: Dict, sim_config: Dict, robs_config: Dict):
        ...
        # for collision-free plan init
        self.sdf_inf = InferenceSingleViewAugSDF(config_dict=sdf_config)
        self.traj_opt = TrajectoryOptimizerWholeBody(
            sdf_infer_map=self.sdf_inf,
            iterations_per_update=traj_opt_cfg["opt_iters"],
            lr_init=traj_opt_cfg["lr_init"],
            lr_decay=traj_opt_cfg["lr_decay"],
            safety_margin=traj_opt_cfg["safety_margin"],
            smoothing=traj_opt_cfg["smoothing"],
            robot_extent_ow=traj_opt_cfg["robot_grid_extent_ow"],
        )
        ...
        self.traj_opt_robo = TrajectoryOptimizerBodySDF(
            sdf_infer_map=self.sdf_inf,
    ...
    def get_optimized_trajectory(
        ...
        """Two modes:
        - first check if collision-free opt.
        - if not, do full scene sdf inference + traj opt.
        - [REMOVED] refine above estimate with collision-free opt.
        """
        ## Preparation ##
        with torch.no_grad():
        ...
        # pc sampling
        sampling_mask = (
            frame_sample.depth_img < self.traj_opt_robo.pc_memory.radius
        ...
        self.traj_opt_robo.pc_memory.pre_update_setup(
            robot_state_new=robot_curr,
            cam_params=self.sdf_inf.cam_params,
            c2w_tform=frame_sample.transform,
        )

        ## Step 1: [robot-sdf] do traj opt. and check if collision-free ##
        traj_opted, is_traj_coll_free = self.traj_opt_robo.optimize_trajectory(
            pc=sampled_pc_pts.pc.reshape(-1, 3),
            obj_det_preds=None,
            traj_pt2ds=traj_init,
            z_value=self.robot_grid_z_offset,
            check_traj_coll_free=True,
        )

        if not is_traj_coll_free:
            ## Step 2: [scene-sdf] if not collision-free, do full scene inference ##
            # update robot state in memory, and then update memory
            robot_state_np = np.array([robot_state.x, robot_state.y], dtype=np.float32)
            self.sdf_inf.memory.set_robot_state(new_state=robot_state_np)
            self.sdf_inf.memory.remove_outdated_obs()

            # using traj provided as arg
            traj_opted = self.traj_opt.optimize_trajectory(
                obj_det_preds=obj_det_preds,
                fs_buffer=fs_buffer,
                traj_pt2ds=traj_opted,
                z_value=self.robot_grid_z_offset,
            )
    ...
    def plan_control_loop(self, obs_move_step_size=200, sim_step_size=20):
        ...
        while True:
            ...
            # control and get updated robot state
            ...
            if (
                ...
                break
            if time.perf_counter() - sim_start_time > cmn.STOP_COND_TIME:
                # failure
...
if __name__ == "__main__":
    ...
    sim = ObjCompDynamicDualModePlannerWithControl(
