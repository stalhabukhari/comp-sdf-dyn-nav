"""
python sim_scene_sdf.py <path-to-config>
"""
...
from planner import TrajectoryOptimizerWholeBody


class ObjCompDynamicPlannerWithControl:
    ...
    def get_optimized_trajectory(
        ...
        # update state in memory, and then update memory
        ...
        while True:
            ...
            if cmn.COUNTER % sim_step_size == 0:
                ...
                if not is_traj_too_short:
                    ...
                    # debug
                    pb_draw_line(
                        traj, env=self.env, life_sec=phys_timestep * sim_step_size + 2
                    )
            ...
            if (
                ...
                break
            if time.perf_counter() - start_time > cmn.STOP_COND_TIME:  # 20 minutes
                # failure
...
if __name__ == "__main__":
    ...
    sim = ObjCompDynamicPlannerWithControl(
